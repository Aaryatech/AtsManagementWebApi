...
public interface GetSupportTaskRepository extends JpaRepository<GetSupportTask, Integer> {
	...
	@Query(value = ("select st.*,e.emp_name,p.project_name from t_support_task st,m_employee e,t_projects p "
			+ "where st.emp_id=:empId and e.emp_id=st.emp_id and p.project_id=st.project_id and st.work_date between :fromDate and :toDate"), nativeQuery = true)
	List<GetSupportTask> getSupportTaskByEmpiId(@Param("empId") int empId, @Param("fromDate") String fromDate,
